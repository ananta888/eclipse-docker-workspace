...
function Ensure-WorkspaceImporterInstalled {
    ...
    $buildScript = Join-Path $RepoRootPath 'tools\workspace-importer\build-workspace-importer.ps1'
...
function Ensure-XmlCatalogEntries {
    ...
    $existingGeneratedNodes = @($catalogRoot.ChildNodes | Where-Object {
        ...
        $_.Attributes['id'] -and
        $_.Attributes['id'].Value.StartsWith('portable-eclipse-auto:')
    })
    ...
    foreach ($entry in $catalogEntries) {
        ...
        $uriNode.SetAttribute('id', "portable-eclipse-auto:$safeId")
...
function Import-ProjectsIntoEclipse {
    ...
    $headlessApp = 'local.win11.portableeclipse.workspaceimporter.application'
...
$cloneScript = Join-Path $root 'scripts\clone-repos.ps1'

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('# Format: <git-url>|<branch>|<target-subdir-under-portable/repos>') | Out-Null
$lines.Add('# Generated by scripts/setup-projects.ps1') | Out-Null
$lines.Add('') | Out-Null
